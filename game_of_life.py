...
import numpy as np
from scipy.signal import convolve2d  # Import convolve2d for optimized neighbor counting
...
def update(frameNum, img, grid, N):
    """
    Conway's Game of Life update function using convolution for efficiency.
    Uses toroidal boundary conditions (grid wraps around).
    """
    # Create binary grid (1 for ON, 0 for OFF)
    binary_grid = (grid == ON).astype(int)
    ...
    newGrid[live_cells & ((neighbor_count < 2) | (neighbor_count > 3))] = OFF

    # Dead cells (value == OFF)
    dead_cells = grid == OFF
    # Dead cells with exactly 3 neighbors become alive
    newGrid[dead_cells & (neighbor_count == 3)] = ON
...
def main():
    ...
    updateInterval = 100  # Increased from 50 to 200 ms for slower animation
    if args.interval:
